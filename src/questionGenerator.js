...
4. DISTRACTORS: All four options are real, plausible NVIDIA techniques/tools or production controls. Wrong answers fail the *specific* stated constraint, not by being made up or obviously reckless. Reject if the correct option is the only professional-sounding answer.
...
6. CALIBRATION: Difficulty matches Professional level (engineer with 2–3 years LLM/agent experience). Reject if it's associate-level recall.
7. WHY-WRONG: Has a "Why <letter> is wrong" line for every wrong option. Four-option question = exactly three "Why X is wrong" lines.
8. ANSWER VARIANCE: Within the batch, correct letters should vary across A/B/C/D. Flag if all answers in the batch are the same letter.
9. SCOPE: Includes "- Scope: general_concept" for broad cert knowledge or "- Scope: nvidia_specific" when NVIDIA tools/platforms are central.
