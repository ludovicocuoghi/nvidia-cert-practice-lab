...
import { buildMistakeEntry, findById, gradeExam, parseExamMarkdown, simulateRun } from "./src/simulator.js";
import {
  appendDomainNote,
  emptyProfile,
  mergeSessionResult,
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/mocks") {
      ...
      const files = await readdir(mocksDir).catch((err) => {
        if (err.code === "ENOENT") return [];
        throw err;
      });
      const mockIds = files
      ...
      const defs = await Promise.all(
        mockIds.map(async (id) => {
          const def = await readJson(join(mocksDir, `${id}.json`));
          return { id: def.id, name: def.name, description: def.description, durationMinutes: def.durationMinutes, passingScorePercent: def.passingScorePercent, questionCount: def.questionIds.length };
        })
      );
      send(res, 200, { mocks: defs });
      return;
    }
    ...
    if (req.method === "GET" && url.pathname === "/api/mock") {
      ...
      const mockDef = await readJson(join(dir, "mocks", `${mockId}.json`));
      const exam = await readExam(dir);
